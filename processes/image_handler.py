...
class Main:
    ...
    def draw_top_banner(self, text):
        ...
        for idx, piece in enumerate(text_grouped):
            ...
            dimensions = ((orig_width - text_width)/2, (((orig_height - len(text_grouped)*100) - text_height)/2) + ((idx + 1)* 80))
            draw.text(dimensions, piece.upper(), color, font=font)
        return img
    ...
    def draw_bottom_banner(self, text):
        ...
        dimensions = ((orig_width-text_width)/2, ((orig_height-text_height)/2) - 110)
        draw.text(dimensions, text, white, font=font)
        
        text_width, text_height = draw.textsize('em até 12x sem juros', font=font)
        dimensions = ((orig_width-text_width)/2, ((orig_height-text_height)/2) - 20)
        draw.text(dimensions, 'em até 12x sem juros', black, font=font)
        return img
    ...
    def check_file_folder_presence(self):
        ...
        if os.path.exists("resources/results"):
            print("Previous images folder present. Will delete contents to proceed and proceed")
