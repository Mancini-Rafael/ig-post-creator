...
class FashionSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item_container in response.css("div.item"):
            has_no_stock = item_container.css('div.item-image-container a.overlay-no-stock h4::text').get()
            ...
            value_raw = item_container.css("div.item-info-container span.item-price::text").get().strip().replace(",00", "").replace("R$", "R$ ")
            if has_no_stock == 'Esgotado':
                print('Ignoring', desc, ' - Esgotado')
                continue
            else:
                print('Added', desc, ' - Disponível')
                items_info.append(dict(url=url, desc=desc, value_raw=value_raw))
